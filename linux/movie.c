...
SE_movie *SE_movie_load(char *path_file)
{
	...
	movie->mdata->numBytes = avpicture_get_size(PIX_FMT_RGB24, movie->mdata->pCodecCtx->width, movie->mdata->pCodecCtx->height);

	movie->mdata->buffer=(uint8_t *)av_malloc(movie->mdata->numBytes*sizeof(uint8_t));

	avpicture_fill((AVPicture *)movie->mdata->pFrameRGB,movie->mdata->buffer,PIX_FMT_RGB24,movie->mdata->pCodecCtx->width,movie->mdata->pCodecCtx->height);
